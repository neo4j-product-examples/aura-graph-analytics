...
// === 1. Load Users ===
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/neo4j-product-examples/aura-graph-analytics/refs/heads/main/better_recommendations/data/order_history.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/neo4j-product-examples/aura-graph-analytics/refs/heads/main/better_recommendations/data/products.csv' AS row
MERGE (p:Product {product_id: toInteger(row.product_id)})
SET p.name = row.product_name;

// === 5. Load Aisles ===
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/neo4j-product-examples/aura-graph-analytics/refs/heads/main/better_recommendations/data/aisles.csv' AS row
MERGE (a:Aisle {aisle_id: toInteger(row.aisle_id)})
SET a.name = row.aisle;

// === 6. Load Departments ===
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/neo4j-product-examples/aura-graph-analytics/refs/heads/main/better_recommendations/data/departments.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/neo4j-product-examples/aura-graph-analytics/refs/heads/main/better_recommendations/data/baskets.csv' AS row
MATCH (o:Order {order_id: toInteger(row.order_id)})
MATCH (p:Product {product_id: toInteger(row.product_id)})
